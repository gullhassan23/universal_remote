...
class SonyTvService implements ITvService {
  ...
  static const _sonyServiceType = 'urn:schemas-sony-com:service:ScalarWebAPI:1';
  static const _defaultApiPort = 80;
  static const _dmrPort =
      52323; // Some TVs expose API on 80, SSDP returns 52323
  ...
  Future<bool> connect(TvDevice device) async {
    ...
    try {
      ...
      if (ok) {
        await _fetchTvIrccCodes(
            _currentDevice!.ip, _currentDevice!.port, _currentDevice!.token!);
  ...
  Future<bool> sendKey(String key) async {
    ...
    try {
      ...
      if (response.statusCode != 200 && port == _dmrPort) {
        response = await _sendIrcc(
            device.ip, _defaultApiPort, device.token!, irccCode);
      }
      return response.statusCode == 200;
    } catch (_) {
      _updateState(TvConnectionState.error);
  ...
  Future<http.Response> _sendIrcc(
      String ip, int port, String psk, String irccCode) async {
